...
I plotted `pop` vs `gdpPercap` in log scale. The colors are modified by using `scale_colour_distiller()`. If I don't use `scale_colour_distiller()`, the default colorbar is blue. After using `scale_colour_distiller()`, the color is green.
